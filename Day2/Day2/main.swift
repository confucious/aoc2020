...
//  Day2
//
//  Created by Jerry Hsu on 12/2/20.
//
...
func parse() {
    for line in input.split(separator: "\n") {
        ...
        let rangeTokens = tokens[0].split(separator: "-")
        let letter = tokens[1].first!
        let password = tokens[2]
        parsedLines.append(Line(
                            minCount: Int(rangeTokens[0])!,
                            maxCount: Int(rangeTokens[1])!,
                            letter: Character(extendedGraphemeClusterLiteral: letter),
...
func part1() {
    ...
    for line in parsedLines {
        let count = line.password.split(separator: line.letter, omittingEmptySubsequences: false).count - 1
        if count >= line.minCount && count <= line.maxCount {
            total += 1
        }
    }
    print(total)
