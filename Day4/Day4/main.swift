...
//  Day4
//
//  Created by Jerry Hsu on 12/4/20.
//
...
enum Field: String {
    ...
    func validWithValue(_ value: String) -> Bool {
        switch self {
        case .byr:
...
struct Passport {
    ...
    init(input: String) {
        ...
        for token in tokens {
            ...
            fields[Field(rawValue: String(parts[0]))!] = String(parts[1])
        }
    }
}
...
func part1() {
    ...
    let passports = lines.map { Passport(input: $0) }
    var count = 0
    for passport in passports {
        if Set(passport.fields.keys).isSuperset(of: Set([.byr, .iyr, .eyr, .hgt, .hcl, .ecl, .pid])) {
            count += 1
        }
    }
    print(count)
...
func part2() {
    ...
    for passport in passports {
        if Set(passport.fields.keys).isSuperset(of: Set([.byr, .iyr, .eyr, .hgt, .hcl, .ecl, .pid]))
            && passport.fields.allSatisfy({ (key: Field, value: String) -> Bool in
                key.validWithValue(value)
            }) {
            count += 1
        }
    }
